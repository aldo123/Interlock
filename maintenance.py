    ...
    def range_stats(self, start_date, end_date):
        with self.cx() as c:
            rows = c.execute("""SELECT duration FROM downtime_events
                ...
                AND end_time IS NOT NULL""", (start_date, end_date)).fetchall()
        total_min = 0
        for r in rows:
            if r[0]:
                p = r[0].split(":")
                try:
                    total_min += int(p[0]) * 60 + int(p[1])
                except:
                    pass
...
class MaintenancePage(ctk.CTkFrame):
    def __init__(self, master, user=None, main_form=None, **kw):
        ...
        self._build()
        if self.main_form and self.main_form.downtime_active:
            self.machine_state = "DOWN"
            self.dt_start = self.main_form.downtime_start
            self.current_dt_id = self.main_form.downtime_id
            self._update_ui_down()
            self._tick_dur()
        if self.main_form:
            if self.main_form.machine_status == "MACHINE DOWN":
                if hasattr(self, 'ms_icon') and self.ms_icon.winfo_exists():
                    self.ms_icon.configure(text_color=RED)
                if hasattr(self, 'ms_text') and self.ms_text.winfo_exists():
                    self.ms_text.configure(text="MACHINE DOWN", text_color=RED)
            elif self.main_form.machine_status == "WAITING MATERIAL":
                if hasattr(self, 'ms_icon') and self.ms_icon.winfo_exists():
                    self.ms_icon.configure(text_color=YELLOW)
                if hasattr(self, 'ms_text') and self.ms_text.winfo_exists():
                    self.ms_text.configure(text="WAITING MATERIAL", text_color=YELLOW)
        self._refresh_all()
        self._tick_clock()
    ...
    def _build_r1(self, parent):
        ...
        kpis = [
            ("TOTAL DOWNTIME", "⏱", "00:00", "hh:mm",  ORANGE, "kpi_dt"),
            ("OCCURRENCES",    "⚡", "0",     "Times",   YELLOW, "kpi_occ"),
            ("MTTR",           "🛠", "0",     "min",     BLUE,   "kpi_mttr"),
            ("MTBF",           "📊", "0",     "hh:mm",   GREEN,  "kpi_mtbf"),
        ]
    ...
    def _build_r2_left(self):
        ...
        self.chart_title = ctk.CTkLabel(hdr, text="HOURLY DOWNTIME", font=("Segoe UI",11,"bold"),
                                        text_color=TEXT)
    ...
    def _build_r3_left(self):
        ...
        lbl("Date Start")
        self.date_start = DateEntry(fb, width=11, background="#1E3A5F", foreground="white",
                                    borderwidth=1, date_pattern="dd/mm/yyyy", font=("Segoe UI",10))
        self.date_start.pack(side="left", padx=(4,12))

        lbl("Date End")
        self.date_end = DateEntry(fb, width=11, background="#1E3A5F", foreground="white",
                                  borderwidth=1, date_pattern="dd/mm/yyyy", font=("Segoe UI",10))
        self.date_end.pack(side="left", padx=(4,12))

        ctk.CTkButton(fb, text="🔄  Refresh", height=30, width=100,
                      fg_color=GREEN, hover_color=GREEN_HOV, text_color=DARK_BG,
        ...
        cols = ("No","Date","Start Time","End Time","Duration","Downtime Type",
                "Root Cause","Corrective Action","Technician")
        ...
        self.tree = ttk.Treeview(tree_frame, columns=cols, show="headings", style="DT.Treeview")
        self.tree.grid(row=0, column=0, sticky="nsew")

        for col in cols:
            self.tree.heading(col, text=col)
            anchor = "center" if col in ("No","Date","Start Time","End Time","Duration") else "w"
    ...
    def _build_r2_right(self):
        ...
        ctk.CTkButton(right, text="✔ SAVE RCA & RETURN TO IDLE", height=48,
                      fg_color="#22C55E", hover_color="#16A34A", text_color="#0B1120",
                      font=("Segoe UI",12,"bold"), corner_radius=8,
                      command=self._return_idle).pack(fill="x", padx=12, pady=(10,8))
    ...
    def _tick_clock(self):
        if not self.winfo_exists():
            return
        self._clk_job = self._safe_after(1000, self._tick_clock)

    def start_downtime(self):
        if not self.winfo_exists():
            return
        if self.machine_state == "DOWN":
            return
        self.machine_state = "DOWN"
        ...
        if self.main_form:
            self.main_form.downtime_active = True
            self.main_form.downtime_start = self.dt_start
            self.main_form.downtime_id = dtid
    ...
    def _tick_dur(self):
        if not self.winfo_exists():
            return
        if self.machine_state != "DOWN" or not self.dt_start:
            return
        d = int((datetime.now()-self.dt_start).total_seconds())
        h,r = divmod(d,3600); m,s = divmod(r,60)
        if hasattr(self, 'dur_lbl') and self.dur_lbl.winfo_exists():
            self.dur_lbl.configure(text=f"Duration   {h:02d}:{m:02d}:{s:02d} (hh:mm:ss)")
        self._dur_job = self._safe_after(1000, self._tick_dur)
    ...
    def _update_ui_down(self):
        ...
        if hasattr(self, 'adi') and self.adi.get("Downtime ID") and self.adi["Downtime ID"].winfo_exists():
    ...
    def _return_idle(self):
        ...
        if not self.dt_start:
            messagebox.showinfo("Info","Tidak ada downtime aktif.", parent=self)
            return
        end = datetime.now()
        ...
        self.current_dt_id = None
        if self.main_form:
            self.main_form.downtime_active = False
            self.main_form.downtime_start = None
            self.main_form.downtime_id = None
            self.main_form.machine_status = "IDLE"
            if hasattr(self.main_form, 'lbl_machine_status') and self.main_form.lbl_machine_status.winfo_exists():
                self.main_form.lbl_machine_status.configure(text="⚪ IDLE", text_color="#CBD5E1")
        if self._dur_job:
            try:
    ...
    def _draw_chart(self):
        ...
        H = cv.winfo_height()
        if W < 20 or H < 20:
            self._safe_after(100, self._draw_chart)
            return

        start = self.filter_start_date
        end   = self.filter_end_date
        data = self.db.range_hourly(start, end)
        ...
        self.chart_title.configure(text=f"HOURLY DOWNTIME ({start} to {end})")
    ...
    def _refresh_kpi(self):
        ...
        total_min, occ = self.db.range_stats(start, end)
        hh = total_min // 60
        mm = total_min % 60
        self.kpi_dt.configure(text=f"{hh:02d}:{mm:02d}")
        ...
        self.total_lbl.configure(text=f"Total : {hh}h {mm}m")

    def _refresh_table(self):
        if not self.winfo_exists():
            return
        self.filter_start_date = self.date_start.get_date().strftime("%Y-%m-%d")
        self.filter_end_date   = self.date_end.get_date().strftime("%Y-%m-%d")
        self.cur_page = 1
        self._load_table()
        self._refresh_kpi()
        self._draw_chart()
    ...
    def _load_table(self):
        ...
        for i,r in enumerate(rows, start=(self.cur_page-1)*self.per_page+1):
            ...
            if st:
                try:
                    date_str = datetime.strptime(st, "%Y-%m-%d %H:%M:%S").strftime("%d/%m/%Y")
                except:
                    pass
        ...
        self._build_pg()
        self._safe_after(50, self._adjust_tree_columns)

    def _build_pg(self):
        for w in self.pg_inner.winfo_children():
            w.destroy()
    ...
    def destroy(self):
        ...
        if self._clk_job:
            try:
                self.after_cancel(self._clk_job)
                self._clk_job = None
            except:
                pass
        super().destroy()